...
class MDTO_Group():
	def __init__(self, cli_args):
		...
		for logfile in glob.glob(os.path.join(self._input_dir,'*_MDTO_log.txt')):
...
class MDTO_Log_Reader():
	...
	def GetStats(self):
		...
		stats['D_HSim'] = Z(stats['TargHR']) - Z(stats['HSimFA'])
		stats['D_LSim'] = Z(stats['TargHR']) - Z(stats['LSimFA'])
		stats['D_Foil'] = Z(stats['TargHR']) - Z(stats['FoilFA'])
		stats['D_AllLure'] = Z(stats['TargHR']) - Z(stats['AllLureFA'])
